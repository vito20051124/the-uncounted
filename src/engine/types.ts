...
export interface EventDef {
  ...
  text: string
  /**
   * ★ 同一幕的替代開場文字。抽中哪一則由 `eventText()` 從 (seed, 日, 分, 地點) 純函數導出。
   *
   * 為什麼需要這個欄位：實測 120 局共觸發 4,438 次事件，其中【前五幕佔 65.9%】，
   * 最上面那一幕（分水井）一局出現 7.9 次——玩家把同樣的五十二個字讀了八遍。
   * 這不是內容不夠，是同一份內容被迫扮演八次不同的時刻。
   *
   * 紀律：variants 只換【周圍的世界】，不換這一幕是什麼。
   * 選項、代價、結果一律不變，因此它永遠不影響任何閘、任何模擬、任何存檔重播
   * （走 `flavor` 串流——rng.ts 從第一天就替「純敘事變體」留了這條線）。
   * 也因此 variants 不得引用任何狀態（不能寫「上禮拜那場火」，因為它可能沒發生過）。
   */
  variants?: string[]
  choices: Choice[]
  src: Src
